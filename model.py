...
This model uses 200 images per class in the
training set.

@author: Sebastian R-Colina (srcolinas@gmail.com,
                             https://github.com/srcolinas)
"""
print('Training shallow model with 200 images per class')
...
with tf.Session(graph=graph0) as sess:
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch % 10 == 0):
            acc_train = get_accuracy(sum_corrects, X_batch, y_batch, splits = 4)
            acc_val = get_accuracy(sum_corrects, X_val, y_val, splits = 4)
